...
```sh
TAG=v0.1.0   # pick the latest release
...
Invoke-WebRequest -Uri "https://github.com/r3dlight/phantom/releases/download/$tag/phantom-$tag-$target.zip" -OutFile phantom.zip
Expand-Archive phantom.zip
"$PWD\phantom-$tag-$target\phantom.exe" --version
```
